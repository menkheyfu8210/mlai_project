...
from skimage.transform import rescale, pyramid_gaussian
...
class FeatureExtractor():
    ...
    def __init__(self) -> None:
        ...
        self.preExtractedTrainFeat = False if len(os.listdir(self.trainFeatPath)) == 0 else True
        self.testFeatPath = './Project/features/test/'
        if not os.path.exists(self.testFeatPath):
            os.makedirs(self.testFeatPath)
        self.preExtractedTestFeat = False if len(os.listdir(self.testFeatPath)) == 0 else True
        self.addFeatPath = './Project/features/add/'
        if not os.path.exists(self.addFeatPath):
            os.makedirs(self.addFeatPath)
        self.preExtractedAddFeat = False if len(os.listdir(self.addFeatPath)) == 0 else True
